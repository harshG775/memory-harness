...
const generateDefaultMemoryTree = ({
    basePath = "memories",
    user,
    rootMeta,
}: {
    ...
    return [
        {
            name: slugify(basePath),
            type: "directory",
            children: [
                mdFile(
                    [
                        `name: ${yamlString("_meta")}`,
...
function printTree(nodes: TreeNode[], label: string): void {
    console.log(label)

    function walk(children: TreeNode[], prefix: string): void {
        children.forEach((child, index) => {
            const isLast = index === children.length - 1
            const connector = isLast ? "└── " : "├── "
            const childLabel = child.type === "directory" ? `${child.name}/` : child.name
            console.log(`${prefix}${connector}${childLabel}`)
...
const MEMORY_DIR_NAME = ".mh"

async function generateMemoryStructure(cwd: string, user: string, rootMeta: string[] = []): Promise<string> {
    const tree = generateDefaultMemoryTree({ user, rootMeta })
    const basePath = join(cwd, MEMORY_DIR_NAME)

    printTree(tree, `${MEMORY_DIR_NAME}/`)

    await mkdir(basePath, { recursive: true })
    for (const child of tree) {
        await writeTree(basePath, child)
    }

    return basePath
}

// CLI entry: `tsx src/lib/memory/default-memories-tree.ts [user]`
// Prints the tree to the console and writes it into a `.mh` folder in the current working directory.
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
    const user = process.argv[2] ?? "the user"

    generateMemoryStructure(process.cwd(), user)
        .then((basePath) => console.log(`\nMemory structure ready at ${basePath}`))
        .catch((err) => {
            console.error(err)
            process.exit(1)
        })
