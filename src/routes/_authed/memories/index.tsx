import { RiAddLine, RiBrainLine, RiDeleteBinLine, RiFileTextLine, RiListCheck2, RiMore2Line } from "@remixicon/react";
...
const SKELETON_ROW_KEYS = ["skeleton-1", "skeleton-2", "skeleton-3", "skeleton-4", "skeleton-5", "skeleton-6"];
			...
			{data.memories.length === 0 ? (
				<Empty>
					<EmptyHeader>
						...
						<EmptyDescription>
							{activeCategory === "all" ? "Nothing has been remembered yet." : "No memories in this category yet."}
						</EmptyDescription>
					</EmptyHeader>
				</Empty>
			) : (
						...
						{groupedMemories.map((group) => {
							...
							return (
								<section key={group.categoryId} className="space-y-2">
									<h2 className="text-sm font-medium text-muted-foreground">{CATEGORY_LABELS[group.categoryId]}</h2>
					...
					{data.total > PAGE_SIZE && (
						<div className="flex items-center justify-between pt-2">
							...
							<div className="flex items-center gap-2">
								...
								<Button variant="outline" size="sm" disabled={!data.hasMore} onClick={() => setPage(currentPage + 1)}>
									Next
								</Button>
							</div>
						</div>
